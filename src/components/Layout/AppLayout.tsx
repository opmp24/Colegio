...
const navItems = [
  ...
  { path: "/crear", label: "Crear", icon: "M12 4v16m8-8H4", staffOnly: true, permission: "eventos" as const },
  ...
  { path: "/tipos-evaluacion", label: "Evaluaciones", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01", staffOnly: true },
...
export default function AppLayout() {
  ...
  const { data: upcomingCount = 0 } = useUpcomingCount(courseIds);

  const items = [
    ...navItems.filter((n) => {
      if (!n.staffOnly) return true;
      if (isTeacher) return true;
      if (n.permission && profile?.permissions?.includes(n.permission)) return true;
      return false;
    }).filter(n => n.path !== "/ajustes"),
    ...(isAdmin ? adminItems : []),
    navItems.find(n => n.path === "/ajustes")!,
  ].filter(Boolean);
    ...
    <div className="min-h-screen bg-school-bg dark:bg-slate-900 text-slate-900 dark:text-slate-100 pb-20 md:pb-0 md:flex">
        ...
        {profile && (
          <div className="px-4 pt-4 pb-2">
            <div className="flex items-center gap-3">
              <Avatar full_name={profile.full_name} icon={profile.avatar_icon} color={profile.avatar_color} size="sm" badge={upcomingCount} />
      ...
      <main className="flex-1 overflow-auto">
        ...
        <header className="md:hidden sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 border-0">
          ...
          <div className="md:hidden bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-2 space-y-1">
            {items.map((item) => (
              <button
                ...
                className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm transition-colors border-0 ${
            ...
            <button onClick={signOut} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 border-0">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
              </svg>
              Cerrar sesión
        ...
        <div className="flex justify-around items-center h-16 max-w-lg mx-auto">
          {items.map((item) => (
            <button
              key={item.path}
              onClick={() => navigate(item.path)}
              className={`flex flex-col items-center gap-0.5 border-0 ${location.pathname === item.path ? "text-primary-600 dark:text-primary-400" : "text-slate-400 dark:text-slate-500"}`}
            >

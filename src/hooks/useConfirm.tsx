  ...
  const dialog = options ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-sm w-full shadow-xl">
        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-2">{options.title}</h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">{options.message}</p>
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex-1 px-4 py-2.5 font-semibold rounded-xl text-white transition-colors ${
              options.variant === "danger" ? "bg-red-600 hover:bg-red-700" : "bg-primary-600 hover:bg-primary-700"
            }`}
          >
